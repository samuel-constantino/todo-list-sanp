...
const tasksListSchema = Joi.object({
  id: Joi.string()
    ...
    .required()
    .messages({
        'string.min': 'ID da lista precisa ter pelo menos 4 caracteres.',
        'string.empty': 'ID da lista não pode ser vazio.',
...
const updateTasksListValidation = (task) => {
  const { error } = tasksListSchema.validate(task);
  if (error) badRequest(error.message);
};

module.exports = { updateTasksListValidation };
